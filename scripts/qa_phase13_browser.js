...
async function run(cfg) {
  ...
  try {
    ...
    await safeCheck(s, 'all mount data sources returned successful HTTP responses', async () => { const src=await page.locator('[data-figure]').evaluateAll(n=>n.map(x=>new URL(x.dataset.source,document.baseURI).href)); const miss=src.filter(x=>!s.data_responses.some(r=>r.url===x&&((r.status>=200&&r.status<300)||r.status===304))); return {pass:miss.length===0,detail:miss.length?miss.join(', '):`${src.length}/17 sources successful`}; });
